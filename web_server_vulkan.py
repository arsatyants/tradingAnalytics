#!/usr/bin/env python3
"""
Vulkan Web Server for Wavelet Analysis Visualization
=====================================================

Provides a web interface using Vulkan-accelerated analysis to:
1. Select cryptocurrency (BTC, ETH, SOL)
2. Run Vulkan wavelet analysis
...
# Configuration
PORT = 8080
# Use Vulkan plot script (Vulkan GPU acceleration or CPU fallback)
PLOT_SCRIPT = 'gpu_wavelet_vulkan.py'
PYTHON_BIN = '.venv/bin/python'
OUTPUT_DIR = 'wavelet_plots_vulkan'
...
<head>
    ...
    <title>Vulkan Wavelet Analysis - Trading Analytics</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <header>
            <h1>🔥 Vulkan Wavelet Analysis</h1>
            <p class="subtitle">Cryptocurrency Trading Analytics with Vulkan GPU Acceleration</p>
        </header>
        ...
        <div class="plots-container" id="plots-container">
            <div class="empty-state">
                ...
                <p>Select a cryptocurrency and click "Run Vulkan Analysis" to generate wavelet decomposition plots.</p>
        ...
        <footer>
            Vulkan-Accelerated Wavelet Analysis | NVIDIA GeForce RTX 4060 Laptop GPU
        </footer>
    </div>
    ...
    <script>
        ...
        async function runAnalysis() {
            ...
            statusEl.innerHTML = '<span class="spinner"></span>Starting Vulkan analysis for ' + selectedCurrency + '/USDT (' + selectedTimeframe + ')...';
            plotsContainer.innerHTML = '<div class="empty-state"><h2>⚙️ Processing...</h2><p>Vulkan analysis started in background...</p></div>';
            ...
            try {
                ...
                statusEl.innerHTML = '<span class="spinner"></span>Vulkan analysis running (Job: ' + jobId.substring(0, 8) + '...)';
                ...
                const poll = async () => {
                    ...
                    if (status.status === 'running') {
                        statusEl.innerHTML = '<span class="spinner"></span>Processing ' + selectedCurrency + '/' + selectedTimeframe + ' (Vulkan)...';
                        ...
                        statusEl.innerHTML = '✓ Vulkan analysis complete! Generated ' + result.plots.length + ' plots' + timingInfo;
        ...
        function displayMetrics(metrics, currency) {
            ...
            if (metrics.gpu_time) {
                html += `
                    <div class="metric-card">
                        <div class="metric-label">Vulkan Processing</div>
            ...
            if (metrics.gpu_platform) {
                html += `
                    <div class="metric-card">
                        <div class="metric-label">Vulkan Device</div>
    ...
    def handle_analyze(self):
        ...
        try:
            ...
            print(f"[JOB] Created job {job_id[:8]} for {currency}/{timeframe} (Vulkan)")
            ...
            self.send_json_response({
                'success': True,
                'job_id': job_id,
                'message': 'Vulkan analysis started'
            })
    ...
    def run_analysis_background(self, job_id, currency, timeframe):
        """Run analysis in background thread using Vulkan script"""
        print(f"[BACKGROUND] Starting Vulkan analysis for {currency}/{timeframe} (job: {job_id[:8]})")
        ...
        try:
            ...
            if result.returncode != 0:
                with job_lock:
                    ...
                    jobs[job_id]['error'] = f'Vulkan script execution failed: {result.stderr}'
                return
            ...
            # Read frequency band metrics from JSON file if available
            ...
            print(f"[BACKGROUND] Vulkan breakdown for {currency}/{timeframe}:")
            ...
            print(f"[BACKGROUND] Vulkan job {job_id[:8]} completed successfully")
            ...
            print(f"[BACKGROUND] Vulkan job {job_id[:8]} failed: {e}")
    ...
    def extract_metrics(self, script_output):
        """Extract price, Vulkan metrics, and frequency band data from script output"""
        metrics = {
            'current_price': None,
            'price_change': None,
            'gpu_time': None,
        ...
        for line in lines:
            ...
            # Extract Vulkan device name
            if 'Using device:' in line or 'Device name:' in line or '[VULKAN]' in line:
                try:
                    if 'Using device:' in line:
                        device_part = line.split('Using device:')[1].strip()
                    elif 'Device name:' in line:
                        device_part = line.split('Device name:')[1].strip()
                    else:
                        # Extract from [VULKAN] prefix line
                        continue
            ...
            if 'Data loading:' in line or 'Data loading (Binance):' in line:
                try:
            ...
            if 'Wavelet compute:' in line or 'Wavelet decomposition:' in line:
                try:
                    time_str = line.split(':')[-1].strip().replace('s', '').split()[0]
                    metrics['wavelet_time'] = f"{float(time_str):.3f}s"
                except:
                    pass
        ...
        # Log extracted Vulkan platform for debugging
        if metrics['gpu_platform']:
            print(f"[DEBUG] Extracted Vulkan device: {metrics['gpu_platform']}")
        else:
            print(f"[DEBUG] No Vulkan device info found in script output")
